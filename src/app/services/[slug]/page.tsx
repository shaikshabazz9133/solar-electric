...
export default async function ServiceDetailPage({ params }: Params) {
  ...
  return (
    <>
      ...
      {service.cardBlocks?.map((block) => (
        <ServiceCardDeck key={block.title} block={block} />
      ))}
      ...
      {showTemplateSections ? (
        <section className="section-y relative bg-ink-50/60">
          <div
            aria-hidden
            className="pointer-events-none absolute inset-0 bg-grid-ink opacity-70"
          />
      ...
      {/* A page carrying its own long-form blocks writes its own closing
          band; the rest fall back to the stock one. */}
      {service.closingCta ? (
        <CtaBand
          title={service.closingCta.title}
          body={service.closingCta.body}
        />
      ) : showTemplateSections ? (
        <CtaBand
